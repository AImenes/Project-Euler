...
int main(){
	int temp, temp2, rest = 0, mente = 0, counter = 99, remember;
	int i, j;
	int sum[100] = {0};
	...
	for(i = 49; i>= 0; i--){
		for (j = 0; j < 100; j++)
		{
			sum[counter] = sum[counter] + number[j][i];
		}
		if(i == 0){
			sum[counter] = sum[counter] + mente;
		}
		else{
			sum[counter] += mente;
			remember = sum[counter];
			rest = sum[counter];
			mente = sum[counter];
